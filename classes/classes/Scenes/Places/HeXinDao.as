...
public class HeXinDao extends BaseContent
{
	...
	public function riverislandVillageStuffLunar():void {
        ...
        outputText("You go deeper in town and discover the whole place is indeed covered in red. The big question now is what should you check out first?");
		menu();
	...
	public function riverislandVillageStuffLunarGifts():void {
		clearOutput();
		outputText("There seems to be a gift exchange going on. Some people are handing out red envelopes, something about luck for the coming year. You get ahold of one and open it hoping for great luck. You open the envelope and... ");
		var Gems:Number = 50;
		Gems += rand(100);
	...
	public function riverislandVillageStuffLunarClothing():void {
		...
		outputText("\n\n\"<i>Exclusive Festival Dresses! Look the part! Buy a dress for the festivities for only 10 spirit stones! C'mon and buy while there's some left!</i>\"");
	...
	public function riverislandVillageStuffLunarChiChi():void {
		...
		//kimono? Traditional Japanese wear? I only ask cause up till now, everything's been primarily chinese only.
		if (flags[kFLAGS.CHI_CHI_FOLLOWER] > 3) {
			outputText("\n\n\"<i>Exclusive Festival Dresses! Look the part! Buy a dress for the festivities for only 50 gems! C'mon and buy while there's some left!</i>\"");
			outputText("\n\nYou put the dress in your bag for now. Now to put it on.");
		}
		...
		function TierI():void {
			...
			if (player.findPerk(PerkLib.JobSoulCultivator) >= 0) {
				...
				addButton(6, "Draco Sweep", buyItem2,consumables.DRASMAN,
						sayLine2(consumables.DRASMAN,"It's a manual for Draco Sweep, this simple technique allows you to unleash an attack that strikes in a wide arc before you.  Perfect for when you are fighting a group of enemies. It also becomes more powerful when you train your body and soul."),
				...
				addButton(7, "Many Birds", buyItem2,consumables.MABIMAN,
						...
						"\n\nWhether you are going to try to go deeper into all that 'soulforce' stuff or not, at least you now have something to begin with.  The name of the manual is strange. It makes you remember something...but what and from where you are not certain.  "
		...
		function TierII():void {
			...
			if (player.findPerk(PerkLib.SoulPersonage) >= 0) {
				...
				addButton(5, "Grandiose Hail of Blades", buyItem2,consumables.HOB2MAN,
						...
						"\n\nWhether you are going to go deeper into all that 'soulforce' stuff or not, at least you now have something to begin with.  The name of the manual is strange, it makes you remember something... but what and from where you're not certain.  "
    ...
    public function mogahenmerchant():void {
        ...
        function Tier1():void {
            menu();
            addButton(0, "BlackPp", buyItem1,consumables.BLACKPP,sayLine1(consumables.BLACKPP,"dog"),onBuyString1).hint("Buy a solid black canine pepper.\nCost: " + String(consumables.BLACKPP.value * 3) + " Gems");
        ...
        function Tier2():void {
            ...
            addButton(3, "BladeGrass", buyItem1,consumables.BLADEGR,sayLine1(consumables.BLADEGR,"mantis"),onBuyString1).hint("Buy blade shaped grass.\nCost: " + String(consumables.BLADEGR.value * 3) + " Gems");
		...
		function Tier4():void {
            ...
            addButton(1, "DrakeHeart", buyItem2,consumables.DRAKHRT,sayLine2(consumables.DRAKHRT,"dragon"),onBuyString2).hint("Buy a drake's heart flower.\nCost: " + String(consumables.DRAKHRT.value / 5) + " Spirit Stones");
    ...
    public function sellFivePurpleCrystalShards():void {
        clearOutput();
        outputText("\"<i>Purple crystal shards. Let me check...yes all of them seems to be in decent state,</i>\" after examination he walks away and returns shortly. \"<i>Here are your ten stones for them.</i>\"");
    ...
    public function sellOneElementalShard():void {
        clearOutput();
        outputText("\"<i>A single elemental shard. It still possesses alot of energy, so that will be three spirit stones,</i>\" he states after examining the shard. Moments after he went to put away the shard Moga returns and gives you three stone.");
    ...
    public function sellOneEIchorVial():void {
        clearOutput();
        outputText("\"<i>A single vial of E-Ichor. It's a super rare item, so... i will give you... twenty spirit stones,</i>\" he states after a very long examination of the vial. Moments after he went to put away the vial Moga returns and gives you twenty stones, with shaky hands as if parting with something precious.");
    ...
    public function sellFiveEIchorVials():void {
        clearOutput();
        outputText("\"<i>Vials of E-Ichor. Let me check...yes all of them seems to be decently preserved,</i>\" after a long examination he walks away and return after an even longer time. \"<i>Here are your hun... hundred stones for them...</i>\"");
    ...
    public function serenamerchant():void {
        ...
        outputText("After entering the shop with a sign saying 'Equipment' over the doors you see a few shelves filled with various weapons, shields, armor and even rarer items like rings and necklaces. Behind the desk located in the central point of the shop you see a woman that seems to have mixed race traits. A shark face and a tail that sometimes shows up on either side of the desk contrasts it's owner's feather covered arms that look similar to bird wings.");
        outputText("\n\n\"<i>Greetings dear customer. I'm Serena and this is my humble shop. Look around and if something catches your eye let me know,</i>\" she says all that almost entirely in one breath after noticing your presence.");
        menu();
	...
	//Google Translation: will require the possibility of levitation or something from the PC to use it
    ...
    public function soulequipmentshelf3():void {
        ...
        //addButton(11, weapons.MACE.shortName, weaponBuy, weapons.MACE);//basic armor made of soulmetal that works with unhindered perk xD 
	...
	public function entranceToRiverDungeon():void {
		...
		if (flags[kFLAGS.NEISA_FOLLOWER] == 0) {
			...
			outputText("Seeing as you came as a team the guards let you in, but not before issuing a final warning.\n\n");
	...
	//awl - wymagać bedzie możliwość lewitacji czy coś od PC aby to używać
	//Google Translation: awl - will require the possibility of levitation or something from the PC to use it
	...
	public function ermaswiftarrowmerchantarcherytraining():void {
		...
		if (flags[kFLAGS.ERMA_ARCHERY_TRAINING] > 0) outputText("\"<i>Need more training? Sure, but it's 10 spirit stones, and as usual paid up front.</i>\"");
			...
			outputText("\"<i>Training new archers is indeed something I do but I don’t do it for free. It'll be 10 spirit stones a session if you want the training.</i>\"");
...
public function soularena():void {
	...
	if (flags[kFLAGS.CHI_CHI_AFFECTION] >= 10 && flags[kFLAGS.CHI_CHI_AFFECTION] < 15) chichiScene.EnterOfTheChiChi();
		...
		//Google Translate: the person managing the arena will be a small nekomata npc
		outputText("\n\n\"<i>Welcome to the Soul Arena. Don't start fights outside of their designated areas or you will be thrown out. If you break any rules here you will be kicked out. Go ahead and pick an area where you want to train or go to the challenges area. Oh and fights in each section cost some spirit stones so be sure to have enough of them as we're not running a charity here,</i>\"");
		outputText(" without wasting time the nekomata overseer of the arena explains to you all that you needed to know about the place and walks away.\n\nSo which of the three possible areas do you want to visit this time?");
		...
		//Google Translate: statuseffect (soulArena) add at the beginning of the fights which will allow you to give other drops, etc. in relation to the meeting of a similar enemy in another place and even easily allows you to make soulforce versions of some races without creating new descriptions of monsters - do it for the challenge mode, in which the fight with the enemy will give a specific drop and you can even play on it to get a reward... e.g. a new camp member ^^
	...
	private function soularenaChallengeSubpages(page:int = 1):void {
		menu();
		if (page == 1) {
			if (player.statusEffectv1(StatusEffects.SoulArenaGaunlets1) > 1) addButtonDisabled(0, "Gauntlet 1", "You've already won this gauntlet today. Come back tomorrow.");
			...
			if (player.statusEffectv1(StatusEffects.SoulArenaGaunlets1) > 0) {
				if (player.statusEffectv2(StatusEffects.SoulArenaGaunlets1) > 1) addButtonDisabled(1, "Gauntlet 2", "You've already won this gauntlet today. Come back tomorrow.");
			...
			//addButton(2, "Gauntlet 3", gauntletchallange3).hint("Fight 5 different enemies one after another.");
			//addButton(3, "Gauntlet 4", gauntletchallange4).hint("Fight 6 different enemies one after another.");
			//addButton(4, "Gauntlet 5", gauntletchallange5).hint("Fight 7 different enemies one after another.");
			//addButton(5, "Gauntlet 6", gaunletchallange6).hint("Fight 8 different enemies one after another.");
    ...
    public function gaunletchallange1fight1():void {
        clearOutput();
        outputText("You register for the gauntlet challenge, and wait patiently until you are called, then move out beyond the arena gate to face your opponent. The crowd is big, you certainly have quite the audience! A man with a necktie and a weird stick screams across the area.\n\n");
	...
	public function gaunletchallange2fight1():void {
        ...
        outputText("\"<i>Ladies and gentlemen, today someone has challenged the second ranking gladiatorial test. Can this would-be hero defeat all three opponents and earn not only a large sum of gems, but the right to brag for a full month as well?! LET'S FIND OUT!</i>\"\n\n");
	...
	public function gaunletchallange2fight3():void {
        ...
        outputText("\"<i>So... you are my opponent, huh? You don’t look like much. By the time I’m done binding you, little pet, you will be calling me mistress!</i>\"\n\n");
	...
	public function gaunletchallange2fight4():void {
		clearOutput();
		gaunletsinbetween();
		outputText("\n\nAs the drider falls defeated the crowd cheers for your victory. That said the battle is far from over yet. A large number of shadows mass behind the opposite gate and you can already guess what's coming up for you next.\n\n");
	...
	public function gaunletchallange2postfight():void {
        ...
        outputText("\"<i>The challenger has defeated all of their opponents! what a miracle! Challenger, you may now exit the arena and claim your prize, well done!</i>\"\n\n");
	...
	private function golemancershop():void {
		...
		if (player.hasStatusEffect(StatusEffects.GolemancerShop)) {
			...
			//What on earth is 'spicing' supposed to mean in this context?
		}
		else {
	...
	private function golemancershop1():void {
		...
		if (player.hasPerk(PerkLib.JobGolemancer)) {
			...
			else addButtonDisabled(13, "'Free Cores?'", "How could such an esteemed and well known golemancer like you ask for freebies? Go smash a few golems to take their cores instead of trying to beg for some from her...")
